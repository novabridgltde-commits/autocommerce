"""Pure TikTok -> UnifiedMessage normalization."""
...
def normalize_tiktok_payload(payload: dict[str, object]) -> UnifiedMessage:
    ...
    sender = build_identity(
        sender_payload,
        external_id=payload.get("sender_id") or payload.get("open_id"),
    ...
    kind = normalize_message_kind(payload.get("type") or payload.get("message_kind") or ("video" if payload.get("attachments") else "text"))

    return UnifiedMessage(
        message_id=ensure_message_id(
            channel=ChannelType.TIKTOK.value,
        ...
        channel_account_id=payload.get("recipient_id") or payload.get("business_account_id"),
