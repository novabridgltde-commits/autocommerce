"""Pure Facebook -> UnifiedMessage normalization."""
...
def normalize_facebook_payload(payload: dict[str, object]) -> UnifiedMessage:
    sender_payload = payload.get("sender") or {}
    recipient_payload = payload.get("recipient") or {}
    external_message_id = payload.get("message_id") or payload.get("id") or payload.get("mid")

    sender = build_identity(
        sender_payload,
        external_id=payload.get("sender_id") or payload.get("psid"),
        display_name=payload.get("display_name"),
    )
    recipient = None
    if recipient_payload or payload.get("recipient_id"):
        recipient = build_identity(recipient_payload, external_id=payload.get("recipient_id") or payload.get("page_id"))

    kind = normalize_message_kind(payload.get("type") or payload.get("message_kind") or ("interactive" if payload.get("interactive") else "text"))
    ...
    return UnifiedMessage(
        ...
        channel_account_id=payload.get("page_id") or payload.get("recipient_id"),
